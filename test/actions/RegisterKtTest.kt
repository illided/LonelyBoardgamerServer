...
internal class RegisterKtTest {
    ...
    fun `User created when all info provided`() {
        ...
        assertTrue(runBlocking { isExistWithSuchVKid("0") })
    }

    @Test
    fun `No additional info provided but user still added`() {
        val myParams = TestParameters()
        myParams["VKAccessToken"] = "token1"
        myParams["address"] = "Moscow"

        runBlocking { AddUser.run(0, myParams) }

        assertTrue(runBlocking { isExistWithSuchVKid("1") })
    }
    ...
    companion object {
        ...
        fun bootstrap() {
            ...
            every { VKConnector.checkToken("token0") } returns "0"
            every { VKConnector.checkToken("token1") } returns "1"
            every { VKConnector.checkToken("token") } returns "2"
